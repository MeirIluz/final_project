...
class XMLConfigManager(IConfigManager):
        ...
        for key in path:            
            ...
            if next_element is None:
                self._logger.log(ConstsStrings.LOG_NAME_ERROR,
                             LoggerMessages.CONFIG_KEY_NOT_FOUND.format(str(self._config_path)), level=logging.ERROR)
                most_likely = None
                close_matches = get_close_matches(key, [el.text.strip() for el in all_elements if el.text], n=1, cutoff=0.6)
                if close_matches:
                    most_likely = close_matches[0]                    
                if most_likely:
                    self._logger.log(ConstsStrings.LOG_NAME_WARNING,
                             LoggerMessages.CONFIG_DID_YOU_MEAN.format(most_likely), level=logging.WARNING)
                else:  
                    self._logger.log(ConstsStrings.LOG_NAME_ERROR,
                             LoggerMessages.CONFIG_NO_MATCHES, level=logging.ERROR)            
                return False
            current = next_element
        return True
    ...
    def _load_xml(self) -> ET.Element:
        try:
            ...
            raise ValueError(f"{ConstsStrings.ERROR_PARSING_XML} {self._config_path}: {e}")
